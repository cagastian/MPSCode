...
ComplexMatrix Magnetization_Op(int spin){
//Exclusively for spin1 -1,0,1 ->0,1,2
	int m=16;
	
	ComplexMatrix S(m,m);
	if(spin!=0 && spin!=2){
	cout<<"Spin must be either 0 (Spin -1) or 2 (Spin 1), you've input Spin="<<spin<<endl;
	return S;
	}
	else{
	int a=-1.0*delta(0,spin), b=delta(2,spin),c=b+a ;
		for(int i=0;i<m;i++){
			if(i==1){S(i,i)=a;}
...
ComplexMatrix Total_Magnetization_Op(){
	ComplexMatrix S(m,m);
	
	S=Magnetization_Op(0)+Magnetization_Op(2);
...
ComplexMatrix Trmuold(ComplexMatrix &A,bool d){
...
ComplexMatrix Trmu(ComplexMatrix &A,bool d){
  ...
  if(d){
	for(beta=0;beta<M;beta++)
	  a(0,0)+=A(0*M,beta*M+beta);
  }else{
      for(beta=0;beta<M;beta++)
	  a(0,0)+=A(beta*M+beta,0*M);
    }
  return a;
}
